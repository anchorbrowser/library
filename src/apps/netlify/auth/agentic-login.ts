...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Netlify using the provided credentials.
...
export default async function loginToNetlify() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.netlify.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Netlify as ${credentials.username}` };
