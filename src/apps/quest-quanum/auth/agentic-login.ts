...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Quest Quanum using the provided credentials.
...
- "true" - Login succeeded: you see the Quest Quanum dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Quest Quanum as ${credentials.username}` };
