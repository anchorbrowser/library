...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Segment using the provided credentials.
...
- "true" - Login succeeded: you see Segment dashboard
...
export default async function loginToSegment() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.segment.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Segment as ${credentials.username}` };
