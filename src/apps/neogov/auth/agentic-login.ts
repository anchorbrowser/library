...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to NEOGOV using the provided credentials.
...
- "true" - Login succeeded: you see the NEOGOV dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to NEOGOV as ${credentials.username}` };
    }
