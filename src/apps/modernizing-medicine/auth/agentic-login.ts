...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Modernizing Medicine using the provided credentials.
...
- "true" - Login succeeded: you see the Modernizing Medicine dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Modernizing Medicine as ${credentials.username}` };
