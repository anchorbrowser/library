...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Epic MyChart using the provided credentials.
...
- "true" - Login succeeded: you see the Epic MyChart dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Epic MyChart as ${credentials.username}` };
