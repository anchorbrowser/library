...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to DrChrono using the provided credentials.
...
- "true" - Login succeeded: you see the DrChrono dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to DrChrono as ${credentials.username}` };
    }
