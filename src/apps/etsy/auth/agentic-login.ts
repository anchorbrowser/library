...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Etsy using the provided credentials.
...
- "true" - Login succeeded: you see Etsy account/shop manager
...
export default async function loginToEtsy(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://www.etsy.com/signin' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Etsy as ${credentials.username}` };
