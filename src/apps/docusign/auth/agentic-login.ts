...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to DocuSign using the provided credentials.

Instructions: Enter email, click Continue, enter password, click "LOG IN", handle 2FA if prompted.
...
export default async function loginToDocuSign() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://account.docusign.com/' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to DocuSign as ${credentials.username}` };
