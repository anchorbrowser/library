...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Google Cloud Console using the provided credentials.
...
- "true" - Login succeeded: you see the Google Cloud Console dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Google Cloud Console as ${credentials.username}` };
