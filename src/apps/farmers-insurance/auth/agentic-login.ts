...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Farmers Insurance using the provided credentials.
...
- "true" - Login succeeded: you see the Farmers Insurance dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Farmers Insurance as ${credentials.username}` };
