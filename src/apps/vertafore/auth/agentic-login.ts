...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Vertafore using the provided credentials.
...
- "true" - Login succeeded: you see the Vertafore dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Vertafore as ${credentials.username}` };
