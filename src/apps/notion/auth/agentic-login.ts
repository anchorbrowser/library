...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Notion using the provided credentials.
...
- "true" - Login succeeded: you see Notion workspace
...
export default async function loginToNotion(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://www.notion.so/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Notion as ${credentials.username}` };
