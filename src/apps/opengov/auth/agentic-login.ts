...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to OpenGov using the provided credentials.
...
- "true" - Login succeeded: you see the OpenGov dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to OpenGov as ${credentials.username}` };
