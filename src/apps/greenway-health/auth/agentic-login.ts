...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Greenway Health using the provided credentials.
...
- "true" - Login succeeded: you see the Greenway Health dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Greenway Health as ${credentials.username}` };
    }
