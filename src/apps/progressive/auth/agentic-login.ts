...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Progressive using the provided credentials.
...
- "true" - Login succeeded: you see the Progressive dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Progressive as ${credentials.username}` };
