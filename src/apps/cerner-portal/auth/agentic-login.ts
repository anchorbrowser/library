...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Cerner Portal using the provided credentials.
...
- "true" - Login succeeded: you see the Cerner Portal dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Cerner Portal as ${credentials.username}` };
    }
