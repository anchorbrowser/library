...
const ConfigSchema = z.object({
  ...
  toolInput: z.object({
    ...
    visitDateFrom: z.string().optional(),
    visitDateTo: z.string().optional(),
  }),
});
...
function buildPrompt(input: Config['toolInput']): string {
  return `Extract Clinical Notes in Cerner Portal:

- patientId: ${input.patientId || 'N/A'}
- visitDateFrom: ${input.visitDateFrom || 'N/A'}
- visitDateTo: ${input.visitDateTo || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          ...
          required: ['filePath', 'noteCount', 'success'],
        },
        maxSteps: 30,
      },
    });
