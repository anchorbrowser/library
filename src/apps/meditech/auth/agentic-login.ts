...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Meditech using the provided credentials.
...
- "true" - Login succeeded: you see the Meditech dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Meditech as ${credentials.username}` };
