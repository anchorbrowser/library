...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Oracle Forms Apps using the provided credentials.
...
- "true" - Login succeeded: you see the Oracle Forms Apps dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Oracle Forms Apps as ${credentials.username}` };
