...
function buildPrompt(input: Config['toolInput']): string {
  return `Automate Data Entry in Oracle Forms Apps:

- formName: ${input.formName || 'N/A'}
- dataPayload: ${input.dataPayload || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          type: 'object',
          properties: {
            recordsEntered: { type: 'number' },
            success: { type: 'boolean' },
          },
          required: ['recordsEntered', 'success'],
        },
        maxSteps: 30,
      },
    });
