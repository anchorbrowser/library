...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Chase Business Online using the provided credentials.
...
- "true" - Login succeeded: you see the Chase Business Online dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Chase Business Online as ${credentials.username}` };
