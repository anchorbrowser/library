...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Asana using the provided credentials.
...
- "true" - Login succeeded: you see the Asana home/inbox
...
export default async function loginToAsana(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.asana.com/-/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Asana as ${credentials.username}` };
