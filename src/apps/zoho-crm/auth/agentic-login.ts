...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Zoho CRM using the provided credentials.
...
- "true" - Login succeeded: you see the Zoho CRM dashboard
...
export default async function loginToZohoCRM(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://accounts.zoho.com/signin' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Zoho CRM as ${credentials.username}` };
