...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Morgan Stanley Online using the provided credentials.
...
- "true" - Login succeeded: you see the Morgan Stanley Online dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Morgan Stanley Online as ${credentials.username}` };
