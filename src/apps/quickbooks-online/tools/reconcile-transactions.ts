...
function buildPrompt(input: Config['toolInput']): string {
  return `Reconcile Bank Transactions in QuickBooks Online:

- accountId: ${input.accountId ?? 'N/A'}
- statementDate: ${input.statementDate ?? 'N/A'}
- statementBalance: ${input.statementBalance ?? 'N/A'}
