...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to QuickBooks Online using the provided credentials.
...
- "true" - Login succeeded: you see the QuickBooks Online dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to QuickBooks Online as ${credentials.username}` };
