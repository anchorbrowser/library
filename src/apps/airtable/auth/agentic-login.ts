...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Airtable using the provided credentials.
...
- "true" - Login succeeded: you see Airtable dashboard
...
export default async function loginToAirtable() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://airtable.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Airtable as ${credentials.username}` };
