...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to CentralSquare using the provided credentials.
...
- "true" - Login succeeded: you see the CentralSquare dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to CentralSquare as ${credentials.username}` };
    }
