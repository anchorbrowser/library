...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to GE Centricity PACS using the provided credentials.
...
- "true" - Login succeeded: you see the GE Centricity PACS dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to GE Centricity PACS as ${credentials.username}` };
