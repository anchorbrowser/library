...
const ConfigSchema = z.object({
  ...
  toolInput: z.object({
    studyDateFrom: z.string().min(1),
    studyDateTo: z.string().min(1),
...
function buildPrompt(input: Config['toolInput']): string {
  return `Export Radiology Metadata in GE Centricity PACS:

- studyDateFrom: ${input.studyDateFrom || 'N/A'}
- studyDateTo: ${input.studyDateTo || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          ...
          required: ['filePath', 'studyCount', 'success'],
        },
        maxSteps: 30,
      },
    });
