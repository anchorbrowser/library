...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Zscaler Admin using the provided credentials.
...
- "true" - Login succeeded: you see the Zscaler Admin dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Zscaler Admin as ${credentials.username}` };
    }
