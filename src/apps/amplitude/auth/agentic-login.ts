...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Amplitude using the provided credentials.

Instructions: Enter email, enter password, click "Log In", handle 2FA if prompted.

Classify:
- "true" - Login succeeded: you see Amplitude dashboard
...
export default async function loginToAmplitude() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://analytics.amplitude.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Amplitude as ${credentials.username}` };
