...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to IBM Mainframe TN3270 using the provided credentials.
...
- "true" - Login succeeded: you see the IBM Mainframe TN3270 dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to IBM Mainframe TN3270 as ${credentials.username}` };
