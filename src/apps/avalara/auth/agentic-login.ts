...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Avalara using the provided credentials.
...
- "true" - Login succeeded: you see the Avalara dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Avalara as ${credentials.username}` };
