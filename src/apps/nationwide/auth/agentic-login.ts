...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Nationwide Portal using the provided credentials.
...
- "true" - Login succeeded: you see the Nationwide Portal dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Nationwide Portal as ${credentials.username}` };
