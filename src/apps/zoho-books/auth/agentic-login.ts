...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Zoho Books using the provided credentials.
...
- "true" - Login succeeded: you see the Zoho Books dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Zoho Books as ${credentials.username}` };
    }
