...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to BambooHR using the provided credentials.
...
- "true" - Login succeeded: you see the BambooHR dashboard
...
export default async function loginToBambooHR(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to BambooHR as ${credentials.username}` };
