...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Ramp using the provided credentials.
...
- "true" - Login succeeded: you see the Ramp dashboard
...
export default async function loginToRamp(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.ramp.com/sign-in' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Ramp as ${credentials.username}` };
