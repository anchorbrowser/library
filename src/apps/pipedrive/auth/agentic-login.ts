...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Pipedrive using the provided credentials.
...
- "true" - Login succeeded: you see the Pipedrive dashboard/pipeline
...
export default async function loginToPipedrive(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.pipedrive.com/auth/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Pipedrive as ${credentials.username}` };
