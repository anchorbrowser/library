...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Geico using the provided credentials.
...
- "true" - Login succeeded: you see the Geico dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Geico as ${credentials.username}` };
    }
