...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Zendesk using the provided credentials.
...
- "true" - Login succeeded: you see Zendesk dashboard
...
export default async function loginToZendesk(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Zendesk as ${credentials.username}` };
