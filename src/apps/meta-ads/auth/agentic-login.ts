...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Meta/Facebook Ads Manager using the provided credentials.
...
- "true" - Login succeeded: you see Ads Manager
...
export default async function loginToMetaAds() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://business.facebook.com' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Meta Ads as ${credentials.username}` };
