...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Duck Creek using the provided credentials.
...
- "true" - Login succeeded: you see the Duck Creek dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Duck Creek as ${credentials.username}` };
    }
