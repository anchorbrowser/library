...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Government Tax Portals using the provided credentials.
...
- "true" - Login succeeded: you see the Government Tax Portals dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Government Tax Portals as ${credentials.username}` };
