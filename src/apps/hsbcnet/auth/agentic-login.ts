...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to HSBCnet using the provided credentials.
...
- "true" - Login succeeded: you see the HSBCnet dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to HSBCnet as ${credentials.username}` };
