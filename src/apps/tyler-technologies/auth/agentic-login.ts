...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Tyler Technologies using the provided credentials.
...
- "true" - Login succeeded: you see the Tyler Technologies dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return {
        success: true,
        message: `Logged in to Tyler Technologies as ${credentials.username}`,
      };
    }
