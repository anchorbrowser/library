...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Amazon Seller Central using the provided credentials.
...
3. Enter password
4. Click "Sign In"
5. Handle 2FA/OTP if prompted

After navigation settles, classify the outcome:
- "true" - Login succeeded: you see Seller Central dashboard
...
export default async function loginToAmazonSeller(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://sellercentral.amazon.com' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Amazon Seller Central as ${credentials.username}` };
