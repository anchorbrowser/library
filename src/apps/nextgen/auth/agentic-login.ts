...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to NextGen using the provided credentials.
...
- "true" - Login succeeded: you see the NextGen dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to NextGen as ${credentials.username}` };
