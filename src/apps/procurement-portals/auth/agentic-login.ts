...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Procurement Portals using the provided credentials.
...
- "true" - Login succeeded: you see the Procurement Portals dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Procurement Portals as ${credentials.username}` };
