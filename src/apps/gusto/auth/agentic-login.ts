...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Gusto using the provided credentials.
...
- "true" - Login succeeded: you see the Gusto dashboard
...
export default async function loginToGusto(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.gusto.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Gusto as ${credentials.username}` };
