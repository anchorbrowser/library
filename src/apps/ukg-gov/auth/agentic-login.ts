...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to UKG Gov using the provided credentials.
...
- "true" - Login succeeded: you see the UKG Gov dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to UKG Gov as ${credentials.username}` };
    }
