...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Brex using the provided credentials.
...
- "true" - Login succeeded: you see the Brex dashboard
...
export default async function loginToBrex(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://dashboard.brex.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Brex as ${credentials.username}` };
