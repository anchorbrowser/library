...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Oracle NetSuite using the provided credentials.
...
- "true" - Login succeeded: you see the Oracle NetSuite dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Oracle NetSuite as ${credentials.username}` };
    }
