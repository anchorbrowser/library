...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to CrowdStrike using the provided credentials.
...
- "true" - Login succeeded: you see the CrowdStrike dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to CrowdStrike as ${credentials.username}` };
