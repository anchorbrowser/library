...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Auth0 Dashboard using the provided credentials.
...
- "true" - Login succeeded: you see the Auth0 Dashboard dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Auth0 Dashboard as ${credentials.username}` };
    }
