...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Citrix-hosted ERP using the provided credentials.
...
- "true" - Login succeeded: you see the Citrix-hosted ERP dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Citrix-hosted ERP as ${credentials.username}` };
