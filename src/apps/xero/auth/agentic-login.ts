...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Xero using the provided credentials.
...
- "true" - Login succeeded: you see the Xero dashboard
...
export default async function loginToXero(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://login.xero.com' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Xero as ${credentials.username}` };
