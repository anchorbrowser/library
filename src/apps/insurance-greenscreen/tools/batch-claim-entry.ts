...
function buildPrompt(input: Config['toolInput']): string {
  return `Batch Claim Entry in Insurance Green-screen Apps:

- claimData: ${input.claimData || 'N/A'}
- batchName: ${input.batchName || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          type: 'object',
          properties: {
            enteredCount: { type: 'number' },
            failedCount: { type: 'number' },
            success: { type: 'boolean' },
          },
          required: ['enteredCount', 'failedCount', 'success'],
        },
        maxSteps: 30,
      },
    });
