...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to GitLab using the provided credentials.
...
- "true" - Login succeeded: you see GitLab dashboard
...
export default async function loginToGitLab() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: `https://${credentials.source || 'gitlab.com'}/users/sign_in` },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to GitLab as ${credentials.username}` };
