...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to AWS Console using the provided credentials.
...
- "true" - Login succeeded: you see the AWS Console dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to AWS Console as ${credentials.username}` };
