...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Kaiser Portal using the provided credentials.
...
- "true" - Login succeeded: you see the Kaiser Portal dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Kaiser Portal as ${credentials.username}` };
