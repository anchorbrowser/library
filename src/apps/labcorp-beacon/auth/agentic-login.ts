...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to LabCorp Beacon using the provided credentials.
...
- "true" - Login succeeded: you see the LabCorp Beacon dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to LabCorp Beacon as ${credentials.username}` };
