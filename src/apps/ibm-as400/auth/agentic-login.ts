...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to IBM AS/400 using the provided credentials.
...
- "true" - Login succeeded: you see the IBM AS/400 dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to IBM AS/400 as ${credentials.username}` };
    }
