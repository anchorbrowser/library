...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Faire using the provided credentials.
...
- "true" - Login succeeded: you see Faire dashboard
...
export default async function loginToFaire(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://www.faire.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Faire as ${credentials.username}` };
