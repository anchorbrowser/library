...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Allstate using the provided credentials.
...
- "true" - Login succeeded: you see the Allstate dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Allstate as ${credentials.username}` };
    }
