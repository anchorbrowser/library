...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Semrush using the provided credentials.
...
- "true" - Login succeeded: you see Semrush dashboard
...
export default async function loginToSemrush() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://www.semrush.com/login/' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Semrush as ${credentials.username}` };
