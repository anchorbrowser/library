...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Twilio using the provided credentials.
...
export default async function loginToTwilio() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://www.twilio.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Twilio as ${credentials.username}` };
