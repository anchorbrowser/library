...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Supplier Portals using the provided credentials.
...
- "true" - Login succeeded: you see the Supplier Portals dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Supplier Portals as ${credentials.username}` };
