...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Stripe Dashboard using the provided credentials.

Instructions:
1. Enter email address
2. Enter password
3. Click "Continue" or "Sign in"
4. Handle 2FA if prompted (SMS or authenticator)

After navigation settles, classify the outcome:
- "true" - Login succeeded: you see the Stripe dashboard
...
export default async function loginToStripe(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://dashboard.stripe.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Stripe as ${credentials.username}` };
