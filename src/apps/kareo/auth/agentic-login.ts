...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Kareo using the provided credentials.
...
- "true" - Login succeeded: you see the Kareo dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Kareo as ${credentials.username}` };
