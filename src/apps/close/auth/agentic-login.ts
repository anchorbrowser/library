...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Close CRM using the provided credentials.
...
- "true" - Login succeeded: you see the Close dashboard/inbox
...
export default async function loginToClose(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.close.com/login/' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Close as ${credentials.username}` };
