...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Sage Intacct using the provided credentials.
...
- "true" - Login succeeded: you see the Sage Intacct dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Sage Intacct as ${credentials.username}` };
