...
function buildPrompt(input: Config['toolInput']): string {
  return `Automate Close Checklist in Sage Intacct:

- closePeriod: ${input.closePeriod || 'N/A'}
- checklistType: ${input.checklistType || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          ...
          required: ['filePath', 'itemCount', 'success'],
        },
        maxSteps: 30,
      },
    });
