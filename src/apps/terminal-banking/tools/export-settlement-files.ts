...
function buildPrompt(input: Config['toolInput']): string {
  return `Export Settlement Files in Terminal Banking Systems:

- settlementDate: ${input.settlementDate || 'N/A'}
- bankCode: ${input.bankCode || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          ...
          required: ['filePath', 'fileCount', 'success'],
        },
        maxSteps: 30,
      },
    });
