...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Terminal Banking Systems using the provided credentials.
...
- "true" - Login succeeded: you see the Terminal Banking Systems dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Terminal Banking Systems as ${credentials.username}` };
