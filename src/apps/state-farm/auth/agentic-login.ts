...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to State Farm using the provided credentials.
...
- "true" - Login succeeded: you see the State Farm dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to State Farm as ${credentials.username}` };
    }
