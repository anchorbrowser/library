...
function buildPrompt(input: Config['toolInput']): string {
  return `Track Prior Authorization in Express Scripts:

- authorizationId: ${input.authorizationId || 'N/A'}
- memberId: ${input.memberId || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          ...
          required: ['status', 'lastUpdated', 'success'],
        },
        maxSteps: 30,
      },
    });
