...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Express Scripts using the provided credentials.
...
- "true" - Login succeeded: you see the Express Scripts dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Express Scripts as ${credentials.username}` };
