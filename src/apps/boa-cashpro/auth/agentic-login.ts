...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Bank of America CashPro using the provided credentials.
...
- "true" - Login succeeded: you see the Bank of America CashPro dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Bank of America CashPro as ${credentials.username}` };
