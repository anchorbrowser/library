...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Lever using the provided credentials.
...
- "true" - Login succeeded: you see the Lever dashboard
...
export default async function loginToLever(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://hire.lever.co/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Lever as ${credentials.username}` };
