...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Drift using the provided credentials.
...
- "true" - Login succeeded: you see Drift inbox/dashboard
...
export default async function loginToDrift(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.drift.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Drift as ${credentials.username}` };
