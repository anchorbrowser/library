...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Practice Fusion using the provided credentials.
...
- "true" - Login succeeded: you see the Practice Fusion dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Practice Fusion as ${credentials.username}` };
