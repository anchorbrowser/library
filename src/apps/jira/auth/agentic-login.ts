...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Jira/Atlassian using the provided credentials.
...
- "true" - Login succeeded: you see a Jira board or project
...
export default async function loginToJira(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://id.atlassian.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Jira as ${credentials.username}` };
