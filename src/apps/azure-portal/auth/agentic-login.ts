...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Azure Portal using the provided credentials.
...
- "true" - Login succeeded: you see the Azure Portal dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Azure Portal as ${credentials.username}` };
