...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Okta Admin using the provided credentials.
...
- "true" - Login succeeded: you see the Okta Admin dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Okta Admin as ${credentials.username}` };
    }
