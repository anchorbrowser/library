...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to SentinelOne using the provided credentials.
...
- "true" - Login succeeded: you see the SentinelOne dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to SentinelOne as ${credentials.username}` };
    }
