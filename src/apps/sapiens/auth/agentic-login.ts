...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Sapiens using the provided credentials.
...
- "true" - Login succeeded: you see the Sapiens dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Sapiens as ${credentials.username}` };
    }
