...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to PayIt using the provided credentials.
...
- "true" - Login succeeded: you see the PayIt dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to PayIt as ${credentials.username}` };
    }
