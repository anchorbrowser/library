...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to BigCommerce using the provided credentials.
...
- "true" - Login succeeded: you see BigCommerce dashboard
...
export default async function loginToBigCommerce(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://login.bigcommerce.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to BigCommerce as ${credentials.username}` };
