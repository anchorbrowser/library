...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Mixpanel using the provided credentials.

Instructions: Enter email, enter password, click "Sign In", handle 2FA if prompted.

After navigation settles, classify:
- "true" - Login succeeded: you see Mixpanel dashboard
...
export default async function loginToMixpanel() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://mixpanel.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Mixpanel as ${credentials.username}` };
