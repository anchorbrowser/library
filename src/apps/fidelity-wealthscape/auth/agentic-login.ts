...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Fidelity Wealthscape using the provided credentials.
...
- "true" - Login succeeded: you see the Fidelity Wealthscape dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Fidelity Wealthscape as ${credentials.username}` };
