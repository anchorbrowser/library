...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Schwab Advisor Center using the provided credentials.
...
- "true" - Login succeeded: you see the Schwab Advisor Center dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Schwab Advisor Center as ${credentials.username}` };
