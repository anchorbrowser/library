...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to New Relic using the provided credentials.
...
- "true" - Login succeeded: you see the New Relic dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to New Relic as ${credentials.username}` };
