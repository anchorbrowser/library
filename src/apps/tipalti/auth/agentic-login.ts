...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Tipalti using the provided credentials.
...
- "true" - Login succeeded: you see the Tipalti dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Tipalti as ${credentials.username}` };
