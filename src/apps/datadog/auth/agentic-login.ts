...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Datadog using the provided credentials.
...
- "true" - Login succeeded: you see Datadog dashboard
...
export default async function loginToDatadog() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: `https://${credentials.source || 'app.datadoghq.com'}` },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Datadog as ${credentials.username}` };
