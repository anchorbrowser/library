...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Palo Alto Admin using the provided credentials.
...
- "true" - Login succeeded: you see the Palo Alto Admin dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Palo Alto Admin as ${credentials.username}` };
