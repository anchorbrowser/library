...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Monday.com using the provided credentials.
...
- "true" - Login succeeded: you see Monday.com boards/workspace
...
export default async function loginToMonday(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://auth.monday.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Monday.com as ${credentials.username}` };
