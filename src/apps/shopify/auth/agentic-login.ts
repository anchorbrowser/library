...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Shopify using the provided credentials.

Instructions:
1. Enter store URL or email
2. Enter password
3. Click "Log in"
4. Handle 2FA if prompted

After navigation settles, classify the outcome:
- "true" - Login succeeded: you see Shopify admin dashboard
...
export default async function loginToShopify() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: `https://${credentials.source || 'accounts.shopify.com/lookup'}` },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Shopify as ${credentials.username}` };
