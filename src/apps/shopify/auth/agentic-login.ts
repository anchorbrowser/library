...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Shopify admin using the provided credentials.
...
export default async function loginToShopify(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://accounts.shopify.com/lookup' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Shopify as ${credentials.username}` };
