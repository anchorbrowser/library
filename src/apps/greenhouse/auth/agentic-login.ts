...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Greenhouse using the provided credentials.
...
- "true" - Login succeeded: you see the Greenhouse dashboard
...
export default async function loginToGreenhouse(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.greenhouse.io/users/sign_in' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Greenhouse as ${credentials.username}` };
