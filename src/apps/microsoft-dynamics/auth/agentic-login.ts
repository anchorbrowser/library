...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Microsoft Dynamics using the provided credentials.
...
- "true" - Login succeeded: you see the Microsoft Dynamics dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Microsoft Dynamics as ${credentials.username}` };
