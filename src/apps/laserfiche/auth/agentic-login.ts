...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Laserfiche using the provided credentials.
...
- "true" - Login succeeded: you see the Laserfiche dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Laserfiche as ${credentials.username}` };
    }
