...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Calendly using the provided credentials.
...
- "true" - Login succeeded: you see Calendly dashboard
...
export default async function loginToCalendly() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://calendly.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to Calendly as ${credentials.username}` };
