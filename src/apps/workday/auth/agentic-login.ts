...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Workday using the provided credentials.
...
- "true" - Login succeeded: you see the Workday home/dashboard
...
export default async function loginToWorkday(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Workday as ${credentials.username}` };
