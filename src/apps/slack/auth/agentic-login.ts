...
Instructions:
1. Enter workspace URL or sign in with email
...
export default async function loginToSlack(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Slack as ${credentials.username}` };
