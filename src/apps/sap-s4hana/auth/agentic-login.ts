...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to SAP S/4HANA using the provided credentials.
...
- "true" - Login succeeded: you see the SAP S/4HANA dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to SAP S/4HANA as ${credentials.username}` };
    }
