...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to CVS Caremark using the provided credentials.
...
- "true" - Login succeeded: you see the CVS Caremark dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to CVS Caremark as ${credentials.username}` };
    }
