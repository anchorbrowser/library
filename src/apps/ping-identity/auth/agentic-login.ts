...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Ping Identity using the provided credentials.
...
- "true" - Login succeeded: you see the Ping Identity dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Ping Identity as ${credentials.username}` };
