...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to PagerDuty using the provided credentials.

Instructions:
1. Enter subdomain if required
2. Enter email
3. Enter password
4. Click "Log in"
5. Handle SSO or 2FA if prompted

After navigation settles, classify the outcome:
- "true" - Login succeeded: you see PagerDuty dashboard
...
export default async function loginToPagerDuty() {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: `https://${credentials.source || 'app.pagerduty.com'}/sign_in` },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') return { success: true, message: `Logged in to PagerDuty as ${credentials.username}` };
