...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Intercom using the provided credentials.
...
- "true" - Login succeeded: you see Intercom inbox/dashboard
...
export default async function loginToIntercom(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.intercom.com/admins/sign_in' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Intercom as ${credentials.username}` };
