...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to GovPilot using the provided credentials.
...
- "true" - Login succeeded: you see the GovPilot dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to GovPilot as ${credentials.username}` };
