...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to InSync EHR using the provided credentials.
...
- "true" - Login succeeded: you see the InSync EHR dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to InSync EHR as ${credentials.username}` };
    }
