...
const ConfigSchema = z.object({
  ...
  toolInput: z.object({
    populationFilter: z.string().optional(),
...
function buildPrompt(input: Config['toolInput']): string {
  return `Extract Demographics in InSync EHR:

- populationFilter: ${input.populationFilter || 'N/A'}
    ...
    const result = await client.agent.task(prompt, {
      ...
      taskOptions: {
        outputSchema: {
          ...
          required: ['filePath', 'patientCount', 'success'],
        },
        maxSteps: 30,
      },
    });
