...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to QuickBooks using the provided credentials.

Instructions:
1. Enter email/user ID
2. Enter password
3. Click "Sign in"
4. Handle 2FA/MFA if prompted

After navigation settles, classify the outcome:
- "true" - Login succeeded: you see the QuickBooks dashboard
...
export default async function loginToQuickBooks(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://accounts.intuit.com/signin' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to QuickBooks as ${credentials.username}` };
