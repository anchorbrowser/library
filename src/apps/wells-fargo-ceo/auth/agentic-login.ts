...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Wells Fargo CEO using the provided credentials.
...
- "true" - Login succeeded: you see the Wells Fargo CEO dashboard
...
export default async function login(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Wells Fargo CEO as ${credentials.username}` };
