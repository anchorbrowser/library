...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to Freshdesk using the provided credentials.
...
- "true" - Login succeeded: you see Freshdesk dashboard
...
export default async function loginToFreshdesk(): Promise<LoginResult> {
  try {
    ...
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to Freshdesk as ${credentials.username}` };
