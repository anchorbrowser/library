...
const RUN_AGENTIC_LOGIN_PROMPT = `Attempt to log in to ClickUp using the provided credentials.
...
- "true" - Login succeeded: you see ClickUp workspace
...
export default async function loginToClickUp(): Promise<LoginResult> {
  try {
    ...
    const result = await client.agent.task(RUN_AGENTIC_LOGIN_PROMPT, {
      ...
      taskOptions: { url: 'https://app.clickup.com/login' },
    });

    const resultStr = String(result).toLowerCase().trim();
    if (resultStr === 'true') {
      return { success: true, message: `Logged in to ClickUp as ${credentials.username}` };
